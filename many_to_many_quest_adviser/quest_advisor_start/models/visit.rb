require_relative("../db/sql_runner")

class Visit

  attr_reader :id
  attr_accessor :user_id, :location_id, :review

  def initialize( options )
    @id = options['id'].to_i if options['id']
    @user_id = options['user_id'].to_i
    @location_id = options['location_id'].to_i
    @review = options['review']
  end

  def save()
    sql = "INSERT INTO visits
    (
      user_id,
      location_id,
      review
    )
    VALUES
    (
      $1, $2, $3
    )
    RETURNING id"
    values = [@user_id, @location_id, @review]
    visit = SqlRunner.run( sql,values ).first
    @id = visit['id'].to_i
  end

  def location()
    sql = "SELECT * FROM locations
    WHERE id = $1"
    values = [@location_id]
    location_data = SqlRunner.run(sql, values)[0]
    return Location.new(location_data)
  end

  def locations()
    sql "SELECT * FROM locations
    INNER JOIN visits
    ON locations.id = visits.location_id
    WHERE users_id = $1"
    values = [@id]
    locations_data_array = SqlRunner.run(sql, values)
    return locations_data_array.map {|data_hash| Location.new(data_hash)}
  end

  def user()
    sql = "SELECT * FROM users
    WHERE id = $1"
    values = [@user_id]
    user_data = SqlRunner.run(sql, values)[0]
    return User.new(user_data)
  end

  def self.all()
    sql = "SELECT * FROM visits"
    visits = SqlRunner.run(sql)
    result = Visit.map_items(visits)
    return result
  end

  def self.delete_all()
    sql = "DELETE FROM visits"
    SqlRunner.run(sql)
  end

  def self.map_items(visit_data)
    result = visit_data.map {|visit| Visit.map(visit)}
    return result
  end

end
